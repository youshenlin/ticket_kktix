var getSelectedTab = (tab) => {
  var tabId = tab[0].id;
  console.log('sdfa =', tabId);
  console.log('sdfa =', tab);
  var sendMessage = (messageObj) =>
    chrome.tabs.sendMessage(tabId, messageObj, (res) => {
  ...
  chrome.storage.local.get('checktest', (items) => {
    if (items.checktest) {
      ...
      document.getElementById('ticketNumber').value =
        items.checktest.ticketNumber;
      document.getElementById('idNumber').value = items.checktest.idNumber;
      document.getElementById('creditNumber').value =
        items.checktest.creditNumber;
      document.getElementById('creditYear').value = items.checktest.creditYear;
      document.getElementById('creditMonth').value =
        items.checktest.creditMonth;
      document.getElementById('creditCheck').value =
        items.checktest.creditCheck;
      document.getElementById('timeCheckHour').value =
        items.checktest.timeCheckHour;
      document.getElementById('timeChecMin').value =
        items.checktest.timeChecMin;
      document.getElementById('timeCheckSecond').value =
        items.checktest.timeCheckSecond;
